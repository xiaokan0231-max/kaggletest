...
# 4 directional prefix convs (fp16)
...
nodes.append(helper.make_node("Min", ["Ls","Rs","Us","Ds"], ["mn"]))
add_const("zero16", np.array(0.0, dtype=np.float16))
nodes.append(helper.make_node("Greater", ["mn","zero16"], ["inside"]))            # bool (filled region incl six)
nodes.append(helper.make_node("Not", ["six_b"], ["notsix"]))
nodes.append(helper.make_node("And", ["inside","notsix"], ["holes_b"]))          # color4
# filled == inside (six implies inside)

# ring via uint8 MaxPool on inside
nodes.append(helper.make_node("Cast", ["inside"], ["filled_u"], to=TensorProto.UINT8))
nodes.append(helper.make_node("MaxPool", ["filled_u"], ["dil_u"], kernel_shape=[3,3], pads=[1,1,1,1], strides=[1,1]))
