...
# 3. ring detection in ONE conv: kernel ones with center -8.
#    cv = (sum of 8 neighbors) - 8*center.  cv==8  <=>  center==0 & all 8 neighbors==1 (ring).
hollow = np.array([[1, 1, 1], [1, -8, 1], [1, 1, 1]], dtype=np.float16).reshape(1, 1, 3, 3)
inits.append(const("hollow", hollow))
nodes.append(helper.make_node("Conv", ["g", "hollow"], ["cv"], kernel_shape=[3, 3], pads=[1, 1, 1, 1]))
inits.append(const("c8", np.array(8.0, dtype=np.float16)))
nodes.append(helper.make_node("Equal", ["cv", "c8"], ["R"]))     # bool ring centers
# Ru = 2 where ring, else 0  (so plus/box MaxPools carry the color-2 value directly)
inits.append(const("two_u8", np.array(2, dtype=np.uint8)))
inits.append(const("zero_u8", np.array(0, dtype=np.uint8)))
nodes.append(helper.make_node("Where", ["R", "two_u8", "zero_u8"], ["Ru"]))  # 0/2

# 4. box dilation (3x3) via MaxPool on uint8 -> 2 inside box
nodes.append(helper.make_node("MaxPool", ["Ru"], ["B"], kernel_shape=[3, 3], pads=[1, 1, 1, 1], strides=[1, 1]))
# 5. plus dilation via vertical + horizontal MaxPool then Max -> 2 on plus
...
# 6. gm = where(box) -> P (2 on plus, 0 on corner) ; else b
nodes.append(helper.make_node("Cast", ["g"], ["b"], to=U8))      # original 0/1
nodes.append(helper.make_node("Cast", ["B"], ["Bb"], to=TensorProto.BOOL))
nodes.append(helper.make_node("Where", ["Bb", "P", "b"], ["gm"]))

# 7. Pad to 30x30 sentinel 255, Equal colors -> output
