...
# crop f32 -> g (900). six via Equal -> bool; six_u uint8
...
# 4 directional prefix-max via uint8 MaxPool
...
nodes.append(helper.make_node("Min", ["L","R","U","D"], ["inside_u"]))   # 1 iff inside (filled), incl six
nodes.append(helper.make_node("Cast", ["inside_u"], ["inside_b"], to=TensorProto.BOOL))

# holes = inside XOR six (six subset of inside -> = inside & ~six)
nodes.append(helper.make_node("Xor", ["inside_b","six_b"], ["holes_b"]))

# dilation of inside, ring = dil XOR inside (inside subset of dil)
...
nodes.append(helper.make_node("Xor", ["dil_b","inside_b"], ["ring_b"]))

# gm uint8 15x15: base where(six,6,8); holes->4; ring->3
