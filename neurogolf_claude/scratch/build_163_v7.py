...
def build(path):
    ...
    for R in (0, 1, 2, 4, 5, 6, 8, 9, 10):
        ...
        TR[R, R % 4] = 1
    inits += [
        c16("EConst", EConst),  # [3,3,11]
        c16("PConst", PConst),  # [3,11,3]
    ...
    # fused selection matrices via single 3-input Einsums (drops 12 intermediates).
    # rowv/rmrow are [1,1,1,11]; batch labels p,q distinct from contraction labels.
    nodes.append(helper.make_node("Einsum", ["rowv", "KR", "EConst"], ["Er"], equation="pqxr,rk,kia->pqia"))  # [1,1,3,11]
    nodes.append(helper.make_node("Einsum", ["rmrow", "KR", "EConst"], ["Ec"], equation="pqxr,rk,kia->pqia"))  # [1,1,3,11]
    nodes.append(helper.make_node("Einsum", ["rowv", "TR", "PConst"], ["Pr"], equation="pqxr,rt,tAi->pqAi"))  # [1,1,11,3]
    nodes.append(helper.make_node("Einsum", ["rmrow", "TR", "PConst"], ["Pc"], equation="pqxr,rt,tAi->pqAi"))  # [1,1,11,3]

    # panel = Er @ g @ Ec^T ; content = Pr @ panel @ Pc^T
    # fuse the two transposes into the Einsums (no EcT/PcT tensors)
    nodes.append(helper.make_node("MatMul", ["Er", "g"], ["keyrows"]))     # [1,1,3,11]
    nodes.append(helper.make_node("Einsum", ["keyrows", "Ec"], ["panel"],
                                  equation="pqia,pqja->pqij"))             # [1,1,3,3]
    nodes.append(helper.make_node("MatMul", ["Pr", "panel"], ["pc1"]))     # [1,1,11,3]
    nodes.append(helper.make_node("Einsum", ["pc1", "Pc"], ["content"],
                                  equation="pqAi,pqBi->pqAB"))             # [1,1,11,11]
