...
def solve(a):
    ...
    for i in range(1,hn+1):
        ...
        if (ymax-ymin)>2 or (xmax-xmin)>2:  # blob too big, shouldn't happen
            pass
        best=None  # (score, paints, color)
        for ty in range(ymax-2,ymin+1):
            for tx in range(xmax-2,xmin+1):
                if not all(ty<=y<ty+3 and tx<=x<tx+3 for y,x in cells): continue
                # window must contain exactly the blob's 0-cells (no foreign holes), and 0-cells outside window adjacent ok
                ...
                for fg,color in keys:
                    for ff in orients(fg):
                        ...
                        if good and not fgout:
                            paints=[(ty+dr-PAD,tx+dc-PAD) for dr in range(3) for dc in range(3) if ff[dr,dc]==1 and 0<=ty+dr-PAD<H and 0<=tx+dc-PAD<W]
                            if best is None: best=(paints,color)
        if best:
            for (yy,xx) in best[0]: out[yy,xx]=best[1]
    return out
ok=True
for split in ['train','test','arc-gen']:
    ...
    for idx,ex in enumerate(d[split]):
        ...
        try: p=solve(a)
        except Exception as e: bad+=1;badidx.append((idx,str(e)[:30]));ok=False;cnt+=1;continue
