...
def find_canvas(a):
    ...
    return best
def get_keys(a,box):  # SIMPLE version from ref_scan (bbox top-left, 3x3)
    ...
    for color,(y0,y1,x0,x1) in keys:
        ...
        out.append(((patch==color).astype(np.int8),int(color)))
    return out
def orients(m):
    for k in range(4):
...
for idx,ex in enumerate(d['arc-gen']):
    ...
    try: p,pl=solve(a)
    except Exception as e: bad.append((idx,'ERR',str(e)[:25])); continue
    if not (p.shape==b.shape and np.array_equal(p,b)):
        from collections import Counter
        winc=Counter((ty,tx) for ty,tx,ff,color in pl)
        multi=sum(1 for v in winc.values() if v>1)
        nd=int((p!=b).sum()) if p.shape==b.shape else -1
        bad.append((idx,'multi' if multi else 'single','windows_w_multi',multi,'diff',nd))
