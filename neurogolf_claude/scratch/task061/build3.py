"""Leaner v3: 5 candidates {5,6,7,8,9}, any-valid Where chain, fill-based validity."""
import math
import numpy as np
import onnx
...
# any valid wins (all valid fills agree)
add_init("zero_hh", np.zeros((H, H), np.float16))
prev = "zero_hh"
for idx, P in enumerate(PERIODS):
    node("Where", [f"valid_{P}", filled[idx], prev], [f"res_{P}"]); vi(f"res_{P}", F16, [H, H])
    prev = f"res_{P}"

add_init("shp1118", np.array([1, 1, H, H], np.int64))
node("Reshape", [prev, "shp1118"], ["fc4"]); vi("fc4", F16, [1, 1, H, H])
padG = G - H
...
params = sum(int(np.prod(i.dims)) for i in inits)
print("nodes", len(nodes), "mem est", tot, "params", params)
