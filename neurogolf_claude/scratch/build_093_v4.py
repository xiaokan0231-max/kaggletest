"""task093 v3: 1D-vector distances/counts (56B), fp16 numerics, bool-output via And."""
import numpy as np
import onnx
...
nodes.append(helper.make_node("Conv", ["input", "collapse_w"], ["g30"]))  # f32 [1,1,30,30]
add_const("cs", np.array([0, 0], np.int64))
add_const("ce", np.array([G, G], np.int64))
add_const("ca", np.array([2, 3], np.int64))
nodes.append(helper.make_node("Slice", ["g30", "cs", "ce", "ca"], ["gf32"]))

add_const("c5", np.array(5, np.float32))
add_const("c0", np.array(0, np.float32))
...
nodes.append(helper.make_node("ReduceSum", ["bf", "axW"], ["rowsum"], keepdims=1))  # [1,1,14,1]
nodes.append(helper.make_node("ReduceSum", ["bf", "axH"], ["colsum"], keepdims=1))  # [1,1,1,14]
...
nodes.append(helper.make_node("And", ["bbelow_b", "nrf_b"], ["above_row_b"]))   # [1,1,14,1]
nodes.append(helper.make_node("And", ["babove_b", "nrf_b"], ["below_row_b"]))
nodes.append(helper.make_node("And", ["bright_b", "ncf_b"], ["left_col_b"]))    # [1,1,1,14]
...
# distances (1D) via inclusive cumsum of region vectors
nodes.append(helper.make_node("CumSum", ["above_row", "axis2"], ["Da"], reverse=1))  # [1,1,14,1]
nodes.append(helper.make_node("CumSum", ["below_row", "axis2"], ["Db"], reverse=0))
nodes.append(helper.make_node("CumSum", ["left_col", "axis3"], ["Dl"], reverse=1))   # [1,1,1,14]
nodes.append(helper.make_node("CumSum", ["right_col", "axis3"], ["Dr"], reverse=0))

# N counts: markers in region, reduce over perpendicular axis
nodes.append(helper.make_node("Mul", ["mf", "above_row"], ["mA"]))   # [1,1,14,14] (392B) - needed
...
nodes.append(helper.make_node("ReduceSum", ["mA", "axH"], ["Na"], keepdims=1))  # [1,1,1,14]
nodes.append(helper.make_node("ReduceSum", ["mB", "axH"], ["Nb"], keepdims=1))
nodes.append(helper.make_node("ReduceSum", ["mL", "axW"], ["Nl"], keepdims=1))  # [1,1,14,1]
nodes.append(helper.make_node("ReduceSum", ["mR", "axW"], ["Nr"], keepdims=1))

add_const("half", np.array(0.5, np.float16))
def fill(D, N, out):
    nodes.append(helper.make_node("Greater", [D, "half"], [out + "_ge1"]))   # 1D bool
    nodes.append(helper.make_node("LessOrEqual", [D, N], [out + "_leN"]))      # broadcasts to grid bool
    nodes.append(helper.make_node("And", [out + "_ge1", out + "_leN"], [out]))  # grid bool
fill("Da", "Na", "fa")
fill("Db", "Nb", "fb")
fill("Dl", "Nl", "fl")
fill("Dr", "Nr", "fr")
...
nodes.append(helper.make_node("Or", ["o3", "fr"], ["out5_b"]))  # bool [1,1,14,14]

# color grid gm (uint8): 5 where out5 else 0; pad to 30x30 with sentinel 255; Equal->one-hot output
nodes.append(helper.make_node("Cast", ["out5_b"], ["out5_u8"], to=TensorProto.UINT8))
add_const("five_u8", np.array(5, np.uint8))
nodes.append(helper.make_node("Mul", ["out5_u8", "five_u8"], ["gm14"]))  # uint8 {0,5}
add_const("pads", np.array([0, 0, 0, 0, 0, 0, 30 - G, 30 - G], np.int64))
add_const("padval_u8", np.array(255, np.uint8))
nodes.append(helper.make_node("Pad", ["gm14", "pads", "padval_u8"], ["gm30"]))  # uint8 [1,1,30,30]
