"""task093: project markers onto a solid 5-band; band extends outward by per-line marker count."""
import numpy as np
import onnx
...
nodes.append(helper.make_node("Slice", ["g30", "crop_starts", "crop_ends", "crop_axes"], ["g"]))
...
nodes.append(helper.make_node("Cast", ["b_bool"], ["bf"], to=TensorProto.FLOAT))
nodes.append(helper.make_node("Cast", ["m_bool"], ["mf"], to=TensorProto.FLOAT))
...
add_const("cW", np.array(float(G), np.float32))
nodes.append(helper.make_node("Equal", ["rowsum", "cW"], ["rowfull_b"]))
nodes.append(helper.make_node("Equal", ["colsum", "cW"], ["colfull_b"]))
nodes.append(helper.make_node("Cast", ["rowfull_b"], ["rowfull"], to=TensorProto.FLOAT))
nodes.append(helper.make_node("Cast", ["colfull_b"], ["colfull"], to=TensorProto.FLOAT))
...
add_const("zf", np.array(0.0, np.float32))
...
for nm in ["above_row", "below_row", "left_col", "right_col"]:
    nodes.append(helper.make_node("Cast", [nm + "_b"], [nm], to=TensorProto.FLOAT))

add_const("ones_grid", np.ones((1, 1, G, G), np.float32))
...
add_const("half", np.array(0.5, np.float32))
def fill(D, N, out):
