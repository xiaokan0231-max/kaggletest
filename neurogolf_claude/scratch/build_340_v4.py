"""task340 v4: fewer [1,1,30,30] grids.

Key fusions vs v3:
  * gp = Where(interior & nz, g, 0): particle-color grid. Wall matches become a
    single Equal(gp, wallc) (no separate &particles And, no extra Cast for the And).
  * noguard: topv = Where(has_top, topc, 0); when topc==0 the value is 0 so a
    spurious match is harmless. (verified 266/266)
All grids uint8/bool (900B); only Conv g32 is f32.
"""
import sys
import numpy as np
import onnx
...
def build(path):
    ...
    nodes.append(helper.make_node("Cast", ["g32"], ["g"], to=U8))         # u8 grid
    ...
    nodes.append(helper.make_node("ReduceMax", ["nzu"], ["colanyu"], axes=[2], keepdims=1))
    nodes.append(helper.make_node("ReduceMax", ["nzu"], ["rowanyu"], axes=[3], keepdims=1))
    ...
    # pmask = interior & nz ; gp = particle colors
    nodes.append(helper.make_node("And", ["interior", "nz"], ["pmask"]))
    nodes.append(helper.make_node("Where", ["pmask", "g", "u0"], ["gp"]))   # u8 [1,1,30,30]

    # wall has-line via Equal(gp, wallc) -> Cast u8 -> ReduceMax
    def wall(name, wallc, axis):
        nodes.append(helper.make_node("Equal", ["gp", wallc], [name + "_eq"]))      # bool grid
        nodes.append(helper.make_node("Cast", [name + "_eq"], [name + "_equ"], to=U8))  # u8 grid
...
if __name__ == "__main__":
    build(sys.argv[1] if len(sys.argv) > 1 else "/tmp/b10_task340_v4.onnx")
