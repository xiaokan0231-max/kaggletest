...
nodes.append(helper.make_node("ReduceMin", ["gf32", "axW"], ["rowmin"], keepdims=1))  # [1,1,14,1] f32
nodes.append(helper.make_node("ReduceMin", ["gf32", "axH"], ["colmin"], keepdims=1))  # [1,1,1,14] f32
nodes.append(helper.make_node("Equal", ["rowmin", "c5"], ["rowfull_b"]))
nodes.append(helper.make_node("Equal", ["colmin", "c5"], ["colfull_b"]))
...
# Counts via MatMul (avoids materializing masked-product grids).
# Na[1,1,1,14] = above_row^T @ mf ;  Nl[1,1,14,1] = mf @ left_col^T
nodes.append(helper.make_node("Transpose", ["above_row"], ["above_rowT"], perm=[0,1,3,2]))  # [1,1,1,14]
nodes.append(helper.make_node("Transpose", ["left_col"], ["left_colT"], perm=[0,1,3,2]))    # [1,1,14,1]
nodes.append(helper.make_node("MatMul", ["above_rowT", "mf"], ["Na"]))   # [1,1,1,14]
nodes.append(helper.make_node("MatMul", ["mf", "left_colT"], ["Nl"]))    # [1,1,14,1]
nodes.append(helper.make_node("ReduceSum", ["mf", "axH"], ["mcol"], keepdims=1))  # total per col
nodes.append(helper.make_node("ReduceSum", ["mf", "axW"], ["mrow"], keepdims=1))  # total per row
nodes.append(helper.make_node("Sub", ["mcol", "Na"], ["Nb"]))   # below = total - above
nodes.append(helper.make_node("Sub", ["mrow", "Nl"], ["Nr"]))   # right = total - left

add_const("bigv", np.array(99.0, np.float16))
def fill(D, region_b, N, out):
    # mask distance to BIG outside region so (<=N) is false there and acts as the >=1 guard
    nodes.append(helper.make_node("Where", [region_b, D, "bigv"], [out + "_dm"]))  # 1D
    nodes.append(helper.make_node("LessOrEqual", [out + "_dm", N], [out]))          # grid
fill("Da", "above_row_b", "Na", "fa")
fill("Db", "below_row_b", "Nb", "fb")
fill("Dl", "left_col_b", "Nl", "fl")
fill("Dr", "right_col_b", "Nr", "fr")
