export const meta = {
  name: 'neurogolf-deepgolf-b13-local',
...
const TARGETS = [
  ...
  { task: 'task303', dep: 15.4 }, { task: 'task125', dep: 15.5 },
  { task: 'task277', dep: 15.5 }, { task: 'task012', dep: 15.6 },
  { task: 'task375', dep: 15.5 }, { task: 'task362', dep: 15.6 },
]
...
3. Else BUILD using the technique above. Save /tmp/b13_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b13_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
