export const meta = {
  name: 'neurogolf-deepgolf-b18-hard',
...
const TARGETS = [
  { task: 'task192', dep: 14.14 }, { task: 'task288', dep: 14.63 },
  { task: 'task161', dep: 14.67 }, { task: 'task154', dep: 14.67 },
  { task: 'task368', dep: 14.69 }, { task: 'task206', dep: 15.03 },
]
...
3. Else BUILD using the technique above. Save /tmp/b18_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b18_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
