export const meta = {
  name: 'neurogolf-deepgolf-b9-local',
...
const TARGETS = [
  { task: 'task066', dep: 12.8 }, { task: 'task367', dep: 13.1 },
  { task: 'task118', dep: 13.4 }, { task: 'task175', dep: 13.8 },
  { task: 'task243', dep: 13.9 }, { task: 'task071', dep: 13.9 },
  { task: 'task090', dep: 14.0 }, { task: 'task011', dep: 14.1 },
  { task: 'task097', dep: 14.1 }, { task: 'task203', dep: 14.1 },
]
...
3. Else BUILD using the technique above. Save /tmp/b9_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b9_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
