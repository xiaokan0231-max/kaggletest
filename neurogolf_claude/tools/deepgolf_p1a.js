...
const TARGETS = [
  { task: 'task076', dep: 12.1 }, { task: 'task018', dep: 13.4 },
  { task: 'task208', dep: 13.6 }, { task: 'task364', dep: 13.8 },
  { task: 'task023', dep: 14.0 }, { task: 'task020', dep: 14.1 },
  { task: 'task044', dep: 14.4 }, { task: 'task279', dep: 14.8 },
]
...
3. Else BUILD using the technique above. Save /tmp/p1a_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/p1a_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
