...
const TARGETS = [
  { task: 'task305', dep: 16.7 }, { task: 'task262', dep: 16.8 },
  { task: 'task139', dep: 16.8 }, { task: 'task381', dep: 16.8 },
  { task: 'task032', dep: 16.9 }, { task: 'task081', dep: 17.3 },
  { task: 'task299', dep: 17.6 }, { task: 'task229', dep: 17.7 },
  { task: 'task052', dep: 17.9 },
]
...
3. Else BUILD using the technique above. Save /tmp/p1f_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/p1f_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
