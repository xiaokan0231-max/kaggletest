...
const TARGETS = [
  { task: 'task044', dep: 14.4 }, { task: 'task251', dep: 14.9 },
  { task: 'task086', dep: 14.9 }, { task: 'task156', dep: 14.9 },
  { task: 'task143', dep: 15.1 }, { task: 'task061', dep: 15.2 },
  { task: 'task277', dep: 15.5 }, { task: 'task192', dep: 14.1 },
]
...
3. Else BUILD using the technique above. Save /tmp/p1b_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/p1b_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
