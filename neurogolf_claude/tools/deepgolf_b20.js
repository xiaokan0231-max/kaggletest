...
const TARGETS = [
  { task: 'task009', dep: 14.23 }, { task: 'task092', dep: 14.26 },
  { task: 'task335', dep: 14.26 }, { task: 'task224', dep: 14.28 },
  { task: 'task358', dep: 14.28 }, { task: 'task328', dep: 14.29 },
  { task: 'task131', dep: 14.30 }, { task: 'task349', dep: 14.31 },
]
...
3. Else BUILD using the technique above. Save /tmp/b20_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b20_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
