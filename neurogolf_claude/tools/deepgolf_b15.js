export const meta = {
  name: 'neurogolf-deepgolf-b15-local',
...
const TARGETS = [
  { task: 'task329', dep: 15.8 }, { task: 'task361', dep: 15.4 },
  { task: 'task301', dep: 15.4 }, { task: 'task277', dep: 15.5 },
  { task: 'task375', dep: 15.5 }, { task: 'task225', dep: 15.7 },
  { task: 'task068', dep: 15.6 }, { task: 'task245', dep: 16.0 },
]
...
3. Else BUILD using the technique above. Save /tmp/b15_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b15_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
