export const meta = {
  name: 'neurogolf-deepgolf-b17-local',
...
const TARGETS = [
  { task: 'task344', dep: 18.19 }, { task: 'task352', dep: 18.19 },
  { task: 'task095', dep: 18.20 }, { task: 'task127', dep: 18.20 },
  { task: 'task220', dep: 18.20 }, { task: 'task230', dep: 18.20 },
  { task: 'task282', dep: 18.20 }, { task: 'task317', dep: 18.20 },
  { task: 'task380', dep: 18.41 }, { task: 'task150', dep: 18.44 },
]
...
3. Else BUILD using the technique above. Save /tmp/b17_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b17_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
