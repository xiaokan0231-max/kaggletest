export const meta = {
  name: 'neurogolf-deepgolf-b7-local',
...
const TARGETS = [
  { task: 'task004', dep: 14.2 }, { task: 'task070', dep: 14.4 },
  { task: 'task359', dep: 14.7 }, { task: 'task163', dep: 14.9 },
  { task: 'task293', dep: 14.9 }, { task: 'task298', dep: 15.4 },
  { task: 'task265', dep: 14.4 }, { task: 'task017', dep: 14.6 },
  { task: 'task196', dep: 14.6 }, { task: 'task278', dep: 14.7 },
]
...
3. Else BUILD using the technique above. Save /tmp/b7_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b7_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
