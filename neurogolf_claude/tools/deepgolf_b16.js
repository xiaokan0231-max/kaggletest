export const meta = {
  name: 'neurogolf-deepgolf-b16-local',
...
const TARGETS = [
  { task: 'task267', dep: 17.9 }, { task: 'task098', dep: 18.19 },
  { task: 'task120', dep: 18.19 }, { task: 'task147', dep: 18.19 },
  { task: 'task151', dep: 18.19 }, { task: 'task171', dep: 18.19 },
  { task: 'task266', dep: 18.19 }, { task: 'task283', dep: 18.19 },
  { task: 'task294', dep: 18.19 }, { task: 'task331', dep: 18.19 },
]
...
3. Else BUILD using the technique above. Save /tmp/b16_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b16_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
