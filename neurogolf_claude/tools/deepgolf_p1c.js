...
const TARGETS = [
  ...
  { task: 'task040', dep: 16.0 }, { task: 'task302', dep: 16.0 },
  { task: 'task369', dep: 16.1 }, { task: 'task007', dep: 16.1 },
  { task: 'task348', dep: 16.1 }, { task: 'task105', dep: 16.1 },
]
...
3. Else BUILD using the technique above. Save /tmp/p1c_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/p1c_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
