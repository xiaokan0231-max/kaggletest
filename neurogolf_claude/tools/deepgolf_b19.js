...
const TARGETS = [
  ...
  { task: 'task088', dep: 14.02 }, { task: 'task313', dep: 14.07 },
  { task: 'task275', dep: 14.16 }, { task: 'task253', dep: 14.21 },
  { task: 'task198', dep: 14.24 }, { task: 'task204', dep: 14.24 },
]
...
3. Else BUILD using the technique above. Save /tmp/b19_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b19_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
