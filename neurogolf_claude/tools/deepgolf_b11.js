export const meta = {
  name: 'neurogolf-deepgolf-b11-local',
...
const TARGETS = [
  { task: 'task168', dep: 15.1 }, { task: 'task397', dep: 15.1 },
  { task: 'task169', dep: 15.2 }, { task: 'task256', dep: 15.2 },
  { task: 'task354', dep: 15.3 }, { task: 'task093', dep: 15.4 },
  { task: 'task361', dep: 15.4 }, { task: 'task301', dep: 15.4 },
  { task: 'task303', dep: 15.4 }, { task: 'task375', dep: 15.5 },
]
...
3. Else BUILD using the technique above. Save /tmp/b11_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b11_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
