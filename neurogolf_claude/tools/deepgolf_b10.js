export const meta = {
  name: 'neurogolf-deepgolf-b10-local',
...
const TARGETS = [
  { task: 'task162', dep: 14.1 }, { task: 'task340', dep: 14.2 },
  { task: 'task338', dep: 14.3 }, { task: 'task288', dep: 14.6 },
  { task: 'task161', dep: 14.7 }, { task: 'task154', dep: 14.7 },
  { task: 'task368', dep: 14.7 }, { task: 'task333', dep: 14.9 },
  { task: 'task074', dep: 15.0 }, { task: 'task206', dep: 15.0 },
]
...
3. Else BUILD using the technique above. Save /tmp/b10_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b10_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
