...
const TARGETS = [
  { task: 'task161', dep: 14.7 }, { task: 'task348', dep: 16.1 },
  { task: 'task273', dep: 16.3 }, { task: 'task353', dep: 16.4 },
  { task: 'task356', dep: 16.5 }, { task: 'task136', dep: 16.5 },
  { task: 'task226', dep: 16.5 }, { task: 'task160', dep: 16.6 },
]
...
3. Else BUILD using the technique above. Save /tmp/p1e_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/p1e_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
