export const meta = {
  name: 'neurogolf-deepgolf-b8-local',
...
const TARGETS = [
  { task: 'task062', dep: 14.8 }, { task: 'task228', dep: 14.9 },
  { task: 'task086', dep: 14.9 }, { task: 'task251', dep: 14.9 },
  { task: 'task287', dep: 15.0 }, { task: 'task166', dep: 15.0 },
  { task: 'task059', dep: 15.1 }, { task: 'task143', dep: 15.1 },
  { task: 'task061', dep: 15.2 }, { task: 'task330', dep: 15.2 },
]
...
3. Else BUILD using the technique above. Save /tmp/b8_${t.task}.onnx. Crop to the common grid size if all examples share one (check first) to shrink tensors.
4. Verify: cd ${REPO} && python neurogolf_claude/tools/verify_local.py ${t.task} /tmp/b8_${t.task}.onnx -> "=== ${t.task}: READY=<bool> points=<float> ===".
