...
dirs_ortho = [(-1, 0, 6), (1, 0, 6), (0, -1, 4), (0, 1, 4)]
dirs_diag = [(-1, -1, 2), (-1, 1, 2), (1, -1, 2), (1, 1, 2)]
...
for i, ex in enumerate(data['train']):
    ...
    for c in range(10):
        ...
        for j in range(1, num+1):
            ...
            S_ortho_all = np.zeros_like(B_mask)
            for dr, dc, _ in dirs_ortho:
                ...
                S_ortho_all |= B_shifted
            
            S_ortho_all = S_ortho_all & (inp != bg) & ~mask
            
            for dirs, is_ortho in [(dirs_ortho, True), (dirs_diag, False)]:
                for dr, dc, k in dirs:
                    B_shifted = np.roll(B_mask, shift=(dr, dc), axis=(0, 1))
                    if dr > 0: B_shifted[:dr, :] = False
                    elif dr < 0: B_shifted[dr:, :] = False
