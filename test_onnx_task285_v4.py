...
for i, ex in enumerate(data['train']):
    ...
    for c in range(10):
        ...
        # Dilate B_c to find adjacent single pixels
        B_dilated = F.conv2d(B_c.view(1, 1, H, W), k_dilate, padding=1).view(H, W) > 0.5
        ...
        # Seed with adjacent single pixels
    ...
    match = torch.equal(out_pred, torch.tensor(out, dtype=torch.float32))
