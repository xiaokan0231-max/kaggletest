...
for i, ex in enumerate(data['train']):
    ...
    for c in range(10):
        ...
        for j in range(1, num+1):
            ...
            for dr, dc, k in dirs:
                # To find single pixels in direction (dr, dc)
                # We can shift the bounding box or the mask?
                # The rule is: single pixel is at (r + dr, c + dc) for some (r, c) in B
                ...
                if S.sum() > 0:
                    ...
                    for ac in ac_colors:
                        # Paste block of color ac at rm + dr*kH, cm + dc*kW
                        tr_rm = rm + dr * kH
                        tr_cm = cm + dc * kW
