    ...
    for c in range(10):
        ...
        for j in range(1, num+1):
            ...
            for dr, dc, k in dirs:
                ...
                # S contains pixels adjacent in this direction
                S = B_shifted & (inp != bg) & ~mask
                
                if S.sum() > 0:
                    ac_colors = np.unique(inp[S])
                    for ac in ac_colors:
                        # Find the single pixels of this color
                        ac_pixels = S & (inp == ac)
                        # We must apply PRECEDENCE!
                        # A pixel in ac_pixels might also be adjacent orthogonally.
                        # If (dr, dc) is diagonal, we must check if it's ALSO orthogonal.
                        # Wait, we can just process orthogonal first, and keep track of stamped pixels!
                        
                        # Actually, we can just stamp it.
                        tr_rm = rm + dr * kH
                        tr_cm = cm + dc * kW
