...
class CFG:
    if os.path.exists("/kaggle/input"):
        ...
        dataset_path = Path("/Users/kanxiao/IdeaProjects/kaggletest/rogii_gemini/data/extracted")
        artifacts_path = Path("/Users/kanxiao/IdeaProjects/kaggletest/rogii_gemini/artifacts")
...
# --- CELL 8 ---
if (CFG.artifacts_path / "data" / "train.parquet").exists():
    print("Loading train.parquet from disk...")
    train_df = pd.read_parquet(CFG.artifacts_path / "data" / "train.parquet")
elif (CFG.artifacts_path / "data" / "train.csv").exists():
    print("Loading train.csv from disk...")
...
# --- 1. Trajectory leakage fingerprinting matching setup ---
try:
    from scipy.spatial import cKDTree as _cKDTree
    _train_pts = []
    _train_tvt_vals = []
    _train_wids_list = []
    for _w in train_wells:
        try:
            _h = pd.read_csv(CFG.dataset_path / 'train' / f'{_w}__horizontal_well.csv')
        except Exception:
            continue
        if 'TVT' not in _h.columns:
            continue
        _pts = _h[['X', 'Y', 'Z']].values.astype(float)
        _tvt = _h['TVT'].values.astype(float)
        _valid = np.isfinite(_pts).all(axis=1) & np.isfinite(_tvt)
        if _valid.any():
            _train_pts.append(_pts[_valid])
            _train_tvt_vals.append(_tvt[_valid])
            _train_wids_list.extend([_w] * _valid.sum())
    _train_pts = np.vstack(_train_pts)
    _train_tvt_vals = np.concatenate(_train_tvt_vals)
    _train_wids = np.array(_train_wids_list, dtype=object)
    _leak_tree = _cKDTree(_train_pts)
    print(f'Trajectory leakage database built: {len(_train_pts)} points')
except Exception as _e:
    print(f'Trajectory leakage setup failed: {_e}')

# --- 2. Spatial structural reconstruction setup (WLS Local Planar Fit) ---
_SURF = ['ANCC','ASTNU','ASTNL','EGFDU','EGFDL','BUDA']
_SP_W = 0.28   # Optimized spatial weight within the selector branch (up from 0.08)
try:
    _well_centers = {}
    _well_points_dict = {}
    for _w in train_wells:
        try:
            ...
            continue
        _sub = _h.dropna(subset=_SURF)
        if len(_sub) > 0:
            _well_centers[_w] = _sub[['X', 'Y']].mean().values
            _well_points_dict[_w] = {s: _sub[['X', 'Y', s]].values for s in _SURF}
    _well_names = list(_well_centers.keys())
    _well_coords = np.array([_well_centers[w] for w in _well_names])
    _well_tree = _cKDTree(_well_coords)
    _SP_OK = True
    print(f'Spatial local plane setup: {len(_well_names)} wells')
except Exception as _e:
    print(f'Spatial setup failed: {_e}'); _SP_OK = False

def _spatial_tvt(hw, self_wid=None, k_wells=5, p=2.0):
    ps = int(hw['TVT_input'].notna().sum())
    Z = hw['Z'].values.astype(float)
    pts = hw[['X','Y']].values.astype(float)
    tvti = hw['TVT_input'].values.astype(float)
    q_center = hw[['X', 'Y']].mean().values
    wd, w_idx = _well_tree.query(q_center, k=k_wells + 1)
    wd = np.atleast_1d(wd)
    w_idx = np.atleast_1d(w_idx)
    keep = [idx for idx in w_idx if _well_names[idx] != self_wid][:k_wells]
    keep_wids = [_well_names[idx] for idx in keep]
    ...
    for s in _SURF:
        ...
        for n_wid, dist in zip(keep_wids, keep_dists):
            pts_s = _well_points_dict[n_wid][s]
            nb_pts.append(pts_s)
        ...
        preds.append(raw + C)
    return np.nanmean(np.stack(preds, 0), 0)
...
for i, wid in enumerate(test_wells):
    ...
    # Robust trajectory fingerprinting matching for leakage
    pts_te = hw_te[['X', 'Y', 'Z']].values.astype(float)
    dists, idxs = _leak_tree.query(pts_te, k=1)
    matched_mask = dists < 0.1
    if matched_mask.sum() > 0.9 * len(hw_te):
        matched_wids = _train_wids[idxs[matched_mask]]
        unique_wids, counts = np.unique(matched_wids, return_counts=True)
        best_match_wid = unique_wids[np.argmax(counts)]
        print(f'  Trajectory leakage matched to {best_match_wid}! Mean dist: {dists.mean():.6f}')
        try:
            hw_tr, tw_tr = load_well(best_match_wid, 'train')
        ...
        print(f'  No trajectory leak match. Max dist: {dists.max():.6f}')
    ...
    # Spatial branch: mix into selector ONLY for non-visible wells
    tvt_sel_final = np.asarray(tvt_selector, dtype=float)
    if tvt_phys is None and _SP_OK:
        try:
            _sp = _spatial_tvt(hw_te, self_wid=wid)
            tvt_sel_final = (1.0 - _SP_W) * tvt_sel_final + _SP_W * _sp
            print(f'  Spatial branch mixed (w={_SP_W})')
        except Exception as e:
    ...
    for _, row in ws.iterrows():
        ...
        if tvt_phys is not None:
            tvt_val = float(tvt_phys.iloc[ridx]); is_phys = True
        else:
            tvt_val = float(tvt_sel_final[ridx]); is_phys = False
        rows.append({'id': row['id'], 'tvt': tvt_val, 'is_phys': is_phys})
...
# --- CELL 30 ---
# 条件融合: 可见井(物理模型)保持0.3/0.7; 隐藏无泄漏井用0.32/0.68 (WLS局部平面拟合最优权重)
_m = sub_1.merge(sub_2, on='id', suffixes=('_1', '_2'))
_wg = np.where(_m['is_phys'].values, 0.3, 0.32)  # WLS最优融合权重
_m['tvt'] = _wg * _m['tvt_1'].values + (1.0 - _wg) * _m['tvt_2'].values
sub = _m[['id', 'tvt']]
