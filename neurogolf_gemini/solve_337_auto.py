...
class TaskNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_colors[:, 8:9, :, :] = torch.max(out_colors[:, 8:9, :, :], x[:, 5:6, :, :])
        out_colors[:, 6:7, :, :] = torch.max(out_colors[:, 6:7, :, :], x[:, 6:7, :, :])
        out_colors[:, 7:8, :, :] = torch.max(out_colors[:, 7:8, :, :], x[:, 7:8, :, :])
        out_colors[:, 5:6, :, :] = torch.max(out_colors[:, 5:6, :, :], x[:, 8:9, :, :])
...
def export_onnx():
    ...
    onnx_path = f"/Users/kanxiao/IdeaProjects/kaggletest/neurogolf/data/working/task337.onnx"
    torch.onnx.export(
        net,
        dummy_input,
        onnx_path,
