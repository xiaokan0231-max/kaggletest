...
for i, ex in enumerate(task_data['train']):
    ...
    # Python pre-computation for start pos
    ...
    for segment in range(4):
        # We need to compute d_obs for all 4 directions and pick based on curr_dir
        ...
        # d_align
        d_align0 = torch.where((tx_pad >= pos_x).any(), tx_pad[tx_pad >= pos_x].min() - pos_x, torch.tensor(1000))
        d_align1 = torch.where((ty_pad >= pos_y).any(), ty_pad[ty_pad >= pos_y].min() - pos_y, torch.tensor(1000))
        d_align2 = torch.where((tx_pad <= pos_x).any(), pos_x - tx_pad[tx_pad <= pos_x].max(), torch.tensor(1000))
        d_align3 = torch.where((ty_pad <= pos_y).any(), pos_y - ty_pad[ty_pad <= pos_y].max(), torch.tensor(1000))
