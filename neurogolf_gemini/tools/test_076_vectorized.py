...
for i, ex in enumerate(task_data['train']):
    ...
    Ar_stack = torch.tensor(np.stack([a for a, c in syms])).unsqueeze(1) # [8, 1, kH, kW]
    Cr_stack = torch.tensor(np.stack([c for a, c in syms])) # [8, 10, kH, kW]
