...
for i, ex in enumerate(task_data['train']):
    ...
    for Ar, Cr in syms:
        ...
        if valid_match.sum() > 0:
            # Draw using conv_transpose2d
            # Cr_t shape is [1, 10, kH, kW]. We need [in_channels=1, out_channels=10, kH, kW]
            # valid_match shape is [1, 1, out_H, out_W]
    ...
    if not np.array_equal(out_final, out_expected):
        ...
        for dy, dx in zip(*diff):
            print(f"({dy}, {dx}): pred {out_final[dy, dx]}, exp {out_expected[dy, dx]}")
