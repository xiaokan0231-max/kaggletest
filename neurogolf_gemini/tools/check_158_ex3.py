...
print("Template:")
for r, c in zip(*np.where(p1_temp)): print(f"  4 at ({r}, {c})")
for r, c in zip(*np.where(p2_temp)): print(f"  1 at ({r}, {c})")
for r, c in zip(*np.where(conn_mask)): print(f"  8 at ({r}, {c})")
...
for i in range(1, n+1):
    ...
    if not np.any(mask & conn_dilated):
        r, c = np.where(mask)
        print(f"  Target 4 at ({r.min()}:{r.max()+1}, {c.min()}:{c.max()+1}) size {len(r)}")
...
for i in range(1, n+1):
    ...
    if not np.any(mask & conn_dilated):
        r, c = np.where(mask)
        print(f"  Target 1 at ({r.min()}:{r.max()+1}, {c.min()}:{c.max()+1}) size {len(r)}")
