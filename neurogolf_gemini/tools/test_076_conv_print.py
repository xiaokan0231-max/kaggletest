...
for i, (Ar, Cr) in enumerate(syms):
    ...
    is_anchor_match = (match_A > A_sum - 0.5) & (match_A < A_sum + 0.5)
    
    if is_anchor_match.sum() > 0:
        print(f"Sym {i} anchor matched at:")
        y_idxs, x_idxs = torch.where(is_anchor_match[0, 0])
        for y, x in zip(y_idxs, x_idxs):
            hint_match = F.conv2d(H_input, Cr_t)[0, 0, y, x].item()
            ones_k = torch.ones(1, 1, kH, kW)
            hint_total = F.conv2d(H_mask, ones_k)[0, 0, y, x].item()
            print(f"  y={y}, x={x}: hint_match={hint_match}, hint_total={hint_total}")
