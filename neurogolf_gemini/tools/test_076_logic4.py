...
for i, ex in enumerate(task_data['train']):
    ...
    # We will compute shifts directly
    for Ar, Cr in syms:
        # Instead of F.conv2d padding, we just loop over shifts manually
        for dy in range(-H + 1, H):
            for dx in range(-W + 1, W):
                # Shift Ar and Cr by dy, dx
                # If dy > 0, we take Ar[:-dy], put at y=dy
                # Actually, simpler to just intersect
                # input window: max(0, dy) to min(H, H+dy)
                y1, y2 = max(0, dy), min(H, H+dy)
                x1, x2 = max(0, dx), min(W, W+dx)
                
                # template window: max(0, -dy) to min(H, H-dy)
                ...
                if np.sum(A_view & Ar_view) == A_sum and np.sum(Ar) == A_sum:
                    # Anchor matches!
