...
for i, ex in enumerate(task_data['train'] + task_data.get('test', [])):
    ...
    # Identify conn_color (the one touching both others)
    conn_color = None
    for c in non_bg:
        mask = (inp == c)
            ...
            for sign_x in [-1, 1]:
                ...
                if np.any(base_c1):
                    for y_c, x_c in T_conn:
                        ...
                        drawn = shift_mask(base_c1, DY_c, DX_c)
                        out_mask |= drawn
                        
    out_pred[out_mask] = conn_color
    print(f"Ex {i} Raw Intersection Match: {np.array_equal(out_pred, out_expected)}")
