...
class TaskNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_colors[:, 4:5, :, :] = torch.max(out_colors[:, 4:5, :, :], x[:, 2:3, :, :])
        out_colors[:, 6:7, :, :] = torch.max(out_colors[:, 6:7, :, :], x[:, 3:4, :, :])
        out_colors[:, 0:1, :, :] = torch.max(out_colors[:, 0:1, :, :], x[:, 4:5, :, :])
        out_colors[:, 5:6, :, :] = torch.max(out_colors[:, 5:6, :, :], x[:, 5:6, :, :])
        out_colors[:, 0:1, :, :] = torch.max(out_colors[:, 0:1, :, :], x[:, 6:7, :, :])
...
def export_onnx():
    ...
    onnx_path = f"/Users/kanxiao/IdeaProjects/kaggletest/neurogolf/data/working/task267.onnx"
    torch.onnx.export(
        net,
        dummy_input,
        onnx_path,
