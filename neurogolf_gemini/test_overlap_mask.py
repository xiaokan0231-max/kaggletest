                    ...
                    for y in range(MAX_SIZE):
                        for x in range(MAX_SIZE):
                            ...
                            if 0 <= sy < MAX_SIZE and 0 <= sx < MAX_SIZE:
                                ...
                                # Match is sum over channels of A_comp_g * A_naked
                                match_val = (A_comp_g[0, :, sy, sx] * A_naked[0, :, y, x]).sum()
                                Match[0, 0, y, x] = match_val
                    
                    Valid_Mask = F.max_pool2d(Match, kernel_size=23, stride=1, padding=11)
                    
                    out_S = torch.max(out_S, S_shift * (Valid_Mask > 0).float())
