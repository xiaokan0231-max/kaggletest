...
class TaskNet(nn.Module):
    ...
    def forward(self, x):
        ...
        B, C, H, W = x.shape
        # Hardcode 30 for arange to avoid dynamic shape tracing issues in ONNX
        MAX_DIM = 30
        ...
        mask_r = torch.max(mask, dim=3, keepdim=True)[0] # [B, 1, MAX_DIM, 1]
        
        prefix_sum = torch.cumsum(mask_r, dim=2)
        suffix_sum = torch.flip(torch.cumsum(torch.flip(mask_r, [2]), dim=2), [2])
        keep_row = (prefix_sum > 0.5) & (suffix_sum > 0.5)
        
        mask_c = torch.max(mask, dim=2, keepdim=True)[0] # [B, 1, 1, MAX_DIM]
        prefix_sum_c = torch.cumsum(mask_c, dim=3)
        suffix_sum_c = torch.flip(torch.cumsum(torch.flip(mask_c, [3]), dim=3), [3])
        keep_col = (prefix_sum_c > 0.5) & (suffix_sum_c > 0.5)
