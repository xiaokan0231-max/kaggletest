...
class TaskNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_colors[:, 5:6, :, :] = torch.max(out_colors[:, 5:6, :, :], x[:, 1:2, :, :])
        out_colors[:, 6:7, :, :] = torch.max(out_colors[:, 6:7, :, :], x[:, 2:3, :, :])
        out_colors[:, 4:5, :, :] = torch.max(out_colors[:, 4:5, :, :], x[:, 3:4, :, :])
        out_colors[:, 3:4, :, :] = torch.max(out_colors[:, 3:4, :, :], x[:, 4:5, :, :])
        out_colors[:, 1:2, :, :] = torch.max(out_colors[:, 1:2, :, :], x[:, 5:6, :, :])
        out_colors[:, 2:3, :, :] = torch.max(out_colors[:, 2:3, :, :], x[:, 6:7, :, :])
        out_colors[:, 7:8, :, :] = torch.max(out_colors[:, 7:8, :, :], x[:, 7:8, :, :])
        out_colors[:, 9:10, :, :] = torch.max(out_colors[:, 9:10, :, :], x[:, 8:9, :, :])
        out_colors[:, 8:9, :, :] = torch.max(out_colors[:, 8:9, :, :], x[:, 9:10, :, :])
...
def export_onnx():
    ...
    onnx_path = f"/Users/kanxiao/IdeaProjects/kaggletest/neurogolf/data/working/task016.onnx"
    torch.onnx.export(
        net,
        dummy_input,
        onnx_path,
